    ...
    <div className="w-full py-28">
      <div className="text-center mb-16">
        ...
        <p className="text-gray-700 dark:text-gray-500">
          Here are all the projects I created while learning about web development. 🧑🏻‍💻
        </p>
      </div> 
        ...
        {fullPortfolio.map((port, index) => (
          <div key={index} className="shadow-xl p-5 rounded-md dark:bg-slate-900">
            ...
            <div className="w-full p-5">
              <h3 className="text-2xl font-semibold text-teal-500 mb-3">{port.name}</h3>
              <div className="flex flex-row mb-3">
                {port.technologies.map((tekno, index) => (
                  <img key={index} src={tekno} alt={port.name} className="w-[30px] h-[30px] lg:w-[40px] lg:h-[40px] m-2" />
                ))}
              </div>
