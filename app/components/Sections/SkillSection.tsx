      ...
      <div className="container mx-auto">
        <div className="flex flex-col md:flex-row-reverse gap-10">
          <div className="w-full md:w-1/4 text-center md:text-right">
            ...
            <p className="text-gray-700 dark:text-gray-400">Here are some tech stacks and tools I have learned, especially in programming.</p>
          </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
                    {techStack.map((skill, index) => (
                      <div key={index} className="relative group flex flex-col items-center p-4 mb-5 hover:translate-y-[-10px] hover:shadow-teal-500 dark:shadow-black shadow-lg transition-all ease-in-out duration-300 rounded-lg">
                        <img src={skill.image} alt={skill.name} className="w-16 h-16 md:w-20 md:h-20 mb-4" />
                        <h4 className="text-lg font-semibold mb-2 text-teal-500 text-center">{skill.name}</h4>
                        <p className="text-sm text-gray-700 dark:text-gray-400">{skill.level}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="tab-panel fade-in-element">
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
                    {tools.map((tool, index) => (
                      <div key={index} className="relative group flex flex-col items-center p-4 mb-5 hover:translate-y-[-10px] hover:shadow-teal-500 dark:shadow-black shadow-lg transition-all ease-in-out duration-300 rounded-lg">
                        <img src={tool.image} alt={tool.name} className="w-16 h-16 md:w-20 md:h-20 mb-4" />
                        <h4 className="text-lg font-semibold mb-2 text-teal-500 text-center">{tool.name}</h4>
                        <p className="text-sm text-gray-700 dark:text-gray-400">{tool.level}</p>
                      </div>
                    ))}
                  </div>
                </div>
